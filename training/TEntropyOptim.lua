...
local TripletEntropyOptim, _ = torch.class('TripletEntropyOptim')
...
function TripletEntropyOptim.weight_bias_parameters(module)
...
function TripletEntropyOptim:__init(model, optState, checkpoint_data)
    assert(model)
...
ce_criterion = nn.CrossEntropyCriterion()

function TripletEntropyOptim:optimize(optimMethod, inputs, embeddings, outputs, targets, criterion, mapper) --, averageUse)
    assert(optimMethod)
    assert(inputs)
    assert(outputs)
    assert(criterion)
    ...
    local err = criterion:forward(outputs)
    local df_do = criterion:backward(outputs)

    --map gradient to the index of input
    local gradient_all = torch.Tensor(numImages, outputs[1]:size(2)):type(inputs:type())
    gradient_all:zero()
    ...
    err = err + ce_criterion:forward(embeddings, targets)
    gradient_all = (gradient_all / 2):add(ce_criterion:backward(embeddings, targets) / 2)

...
local HingeOptim, _ = torch.class('HingeOptim')
...
function HingeOptim.weight_bias_parameters(module)
...
function HingeOptim:__init(model, optState, checkpoint_data)
    assert(model)
...
extraModel:add(nn.PairwiseDistance())

function HingeOptim:optimize(optimMethod, inputs, output, targets, criterion, mapper)
    assert(optimMethod)
    assert(output)
    assert(criterion)

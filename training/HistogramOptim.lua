...
local HistogramOptim, _ = torch.class('HistogramOptim')
...
function HistogramOptim.weight_bias_parameters(module)
...
function HistogramOptim:__init(model, optState, checkpoint_data)
    assert(model)
...
function HistogramOptim:optimize(optimMethod, inputs, output, targets, criterion, mapper)
    assert(optimMethod)
    assert(inputs)
    assert(criterion)
    ...
    local res = cosine:forward(output)
    local err = criterion:forward(res, targets)

    local df_do = criterion:backward(res, targets)
    df_do = cosine:backward(output, df_do)

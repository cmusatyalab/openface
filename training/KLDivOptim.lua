...
local KLDivOptim, _ = torch.class('KLDivOptim')
...
function KLDivOptim.weight_bias_parameters(module)
...
function KLDivOptim:__init(model, optState, checkpoint_data)
    assert(model)
...
function KLDivOptim:optimize(optimMethod, inputs, output, targets, criterion, mapper)
    assert(optimMethod)
    assert(output)
    assert(criterion)

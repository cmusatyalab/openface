...
local DistanceRatioOptim, _ = torch.class('DistanceRatioOptim')
...
function DistanceRatioOptim.weight_bias_parameters(module)
...
function DistanceRatioOptim:__init(model, optState, checkpoint_data)
    assert(model)
...
extraModel:add(distanceModel)

function DistanceRatioOptim:optimize(optimMethod, inputs, output, criterion, mapper) --, averageUse)
    assert(optimMethod)
    assert(inputs)
    assert(criterion)
    ...
    local err = criterion:forward(res)

local QuadrupletCriterion, parent = torch.class('nn.QuadrupletCriterion', 'nn.Criterion')

function QuadrupletCriterion:__init(alpha)
    parent.__init(self)
...
function QuadrupletCriterion:updateOutput(input)
...
function QuadrupletCriterion:updateGradInput(input)
    local a = input[1] -- ancor
    local p = input[2] -- positive
    local n = input[3] -- negative
    local N = a:size(1)

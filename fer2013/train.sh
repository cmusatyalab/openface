...
if [ ! -d $ALIGNED_DIR/train ]; then
    python ../util/align-dlib.py $RAW_DIR/train align  outerEyesAndNose $ALIGNED_DIR/train --size 48 --rgb 0 \
        --fallbackLfw $RAW_DIR/train
fi
...
th main.lua -data $ALIGNED_DIR/train -modelDef $WORK_DIR/model/nn4.small2.def.lua -cache $WORK_DIR/data/cache  \
 -save $RESULT_DIR  -nDonkeys 2  -cuda -cudnn -peoplePerBatch 7 -imagesPerPerson 5 -testPy ../evaluation/classify.py \
